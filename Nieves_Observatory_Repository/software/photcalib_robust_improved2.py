            ...
            with fits.open(flat_file) as hdul:
                ...
                for hdu in hdul:
                    if hasattr(hdu, 'data') and isinstance(hdu.data, np.ndarray) and hdu.data.ndim == 2:
                        flat_data = hdu.data.astype(float)
                        break
...
def select_and_verify_reference_stars(fits_files: List[str], args: argparse.Namespace,
    ...
    for i in range(len(x_refined)):
        ...
        # Check SNR with adjusted threshold
        if phot['snr'][i] < effective_min_snr:
            continue
        
        # Check flux ratio with target using adjusted limit
        flux_ratio = phot['flux'][i] / target_flux if target_flux > 0 else np.inf
        if flux_ratio > effective_flux_ratio_limit or flux_ratio < 1.0/effective_flux_ratio_limit:
            continue
        
        # Calculate quality score
        # Adjust scoring to account for the fact that we may have relaxed criteria
        snr_score = min(phot['snr'][i] / max(effective_min_snr * 2, 100.0), 1.0)
        flux_score = 1.0 - abs(np.log10(flux_ratio)) / np.log10(effective_flux_ratio_limit) if flux_ratio > 0 else 0
